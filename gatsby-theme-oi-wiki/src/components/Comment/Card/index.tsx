import { Avatar, Card, CardActions, CardContent, CardHeader, CircularProgress, IconButton } from '@material-ui/core';
    ...
    <Card variant="outlined" className={classes.commentMargin}>
      ...
      <CardActions>
        <ReactionButton
          ...
          addReaction={async () => {
            await props.addReaction(commentID, 'like');
          }}
          removeReaction={async () => {
            await props.removeReaction(commentID, 'like');
          }}
          users={like?.users ?? []}
        />
        <ReactionButton
          ...
          addReaction={async () => {
            await props.addReaction(commentID, 'unlike');
          }}
          removeReaction={async () => {
            await props.removeReaction(commentID, 'unlike');
          }}
          users={unlike?.users ?? []}
        />
        <ReactionButton
          ...
          addReaction={async () => {
            await props.addReaction(commentID, 'heart');
          }}
          removeReaction={async () => {
            await props.removeReaction(commentID, 'heart');
          }}
          users={heart?.users ?? []}
        />
      </CardActions>
